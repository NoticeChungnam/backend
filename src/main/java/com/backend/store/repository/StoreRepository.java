...
public interface StoreRepository extends JpaRepository<Store, Long> {
    @Query("SELECT new com.backend.store.domain.response.StoreResponse(s.storeId, s.storeName, s.latitude, s.longitude, s.priceRange, s.storeUrl) " +
            ...
            "ORDER BY RAND() " +
            "LIMIT 2")
    List<StoreResponse> findRecommendStores(@Param("priceRange") PriceRange priceRange, @Param("area") Area area, @Param("category") CategoryMajor category);
    ...
    @Query("SELECT new com.backend.store.domain.response.StoreResponse(s.storeId, s.storeName, s.latitude, s.longitude, s.priceRange, s.storeUrl) " +
            ...
            "ORDER BY RAND() " +
            "LIMIT 1")
    StoreResponse findNewStore(@Param("targetStoreId") Long targetStoreId, @Param("anotherStoreId") Long anotherStoreId);
}
